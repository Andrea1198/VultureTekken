...
def drawgrid(X, Y, l, h, screen):
    import pygame
    screen.fill((255,255,255))
    rett    = pygame.Surface((l, h))
    for i, x in enumerate(X):
        screen.blit(rett, (x, Y[i]))
...
def select_player(WIDTH, HEIGHT):
    import pygame
    ...
    (x, y, length, height)    = create_grid(WIDTH, HEIGHT//4, n_col, n_row, 20, 20, 0, 3*HEIGHT//4)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
        x = 1
        drawgrid(x, y, length, height, screen)
    #
